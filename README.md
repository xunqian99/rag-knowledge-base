...
Hit@1 只从 89.2% 提到 91.5%,提升 2.3 个百分点 ——
明显小于精排带来的收益(6.2 个百分点)。
...
精排把 Hit@1 从 91.5% 拉到 **97.7%**,MRR 从 0.9490 提到 **0.9885**。

其中提升最猛的是**改写型:Hit@1 从 80.0% 直接到 97.5%**,MRR 从 0.8717 到 0.9875。
...
>
> `eval/report.md` 里另外记录了两件事:命中判定的口径,以及一处已知的假阴性
> (精排给出的答案其实正确,但因为措辞不同没通过字面匹配)。
