...
|---|---|---|---|---|
| 仅向量 | 84.6% | 100.0% | 0.9153 | 35s |
| 仅 BM25 | 92.3% | 98.5% | 0.9526 | <1s |
| **混合** | 92.3% | **100.0%** | 0.9558 | 37s |
| **混合 + 精排** | **98.5%** | **100.0%** | **0.9923** | 76s |
...
| 配置 | 直接型 | 改写型 | 精确型 |
|---|---|---|---|
| 仅向量 | 0.9338 | 0.8821 | 0.9200 |
| 仅 BM25 | **1.0000** | **0.8458** | **1.0000** |
...
BM25 在**直接型和精确型上 Hit@1 都是满分 100%**,但在**改写型上只有 75%** ——
用户换一种说法提问,它完全找不到。

向量则相反:改写型还有 80%,但直接型和精确型都明显不如 BM25(87.7% / 84.0%)——
**语义检索把精确的字面信息压缩掉了**。

**结论二:混合解决的是「找得到」,不是「排得准」**

混合把 Hit@5 从 98.5% 补到了 **100%**(召回广度问题解决了),
但 Hit@1 停在 92.3% 没有提升 —— 甚至和 BM25 单路一样。
...
精排把 Hit@1 从 92.3% 拉到 **98.5%**,MRR 从 0.9558 提到 **0.9923**。

其中提升最猛的是**改写型:Hit@1 从 82.5% 直接到 100%**,MRR 从 0.8938 到 1.0000。
...
> **关于评测的诚实说明**
>
> Hit@5 四种配置都在 98% 以上,说明这个任务对当前语料
> (6 份文档、45 个分块)**偏简单了,存在天花板效应**。
...
或者用命令行跑 `EvaluationMain`。报告输出到 `eval/report.md`。
...
5. **精排分数阈值未实施**:无关问题目前仍会调用一次大模型,
   理论上可以用精排分数提前判断"资料里没有答案"。阈值需要评测数据确定。
