...
flowchart TB
    Q["用户提问"] --> C{"缓存命中?"}
    C -->|是| FAST["直接返回<br/>约 20ms,不检索不调模型"]
    C -->|否| EMB["问题向量化"]

    EMB --> V["① 向量召回 TOP 50<br/>查 pgvector"]
    V --> B["② BM25 召回 TOP 50<br/>直接拿问题原文查 Elasticsearch"]
    B --> F["RRF 融合<br/>Σ 1/(k+rank),k=60"]
    F --> R["Cross-encoder 精排<br/>候选截断到 20 条 → 取 TOP 3"]
    R --> P["组装 Prompt<br/>资料编号 + 问题"]
    P --> L["调用大模型<br/>同步接口等生成完 / 流式接口边生成边推"]
    L --> OUT["答案 + 引用来源<br/>只保留答案真正引用到的"]
    OUT --> SAVE["落库 + 写缓存<br/>降级产物不写"]
```

两个容易看错的地方,这里特意画清楚了:

- **两路召回是串行的,不是并行的。** 并行最多省 20ms,而整条链路是 500ms 量级,
  为了 4% 的收益引入线程池和并发复杂度不划算(详见决策记录 014)。
- **BM25 不需要向量化。** 它直接拿问题原文去查 ES,所以虽然排在向量召回后面,
  但它本身不依赖上一步的结果 —— 串行只是编排上的选择,不是数据依赖。
...
| **降级状态可见** | 后端把**降级清单**一路传上来(降了哪些环节、影响是什么),前端按类型分别展示 —— 用户有权知道自己看到的是完整链路的结果还是降级产物 |
...
| POST | `/api/chat` | 提问,返回答案 + 引用来源 + 降级清单 |
...
`degraded` 为 `true` 表示本次结果经过了降级。**具体降了哪些环节看 `degradations`** ——
它是一个清单,每项带 `code` 和 `message`:

| code | 含义 | 对用户的影响 |
|---|---|---|
| `CHAT_MODEL_UNAVAILABLE` | 大模型不可用 | **返回的不是答案,是检索到的原文清单** |
| `RECALL_CHANNEL_DOWN` | 某一路召回通道失败 | 检索范围缩小,可能漏掉部分结果 |
| `RERANK_UNAVAILABLE` | 重排序不可用 | 答案正常,只是排序质量下降 |

**为什么不只返回一个布尔值**:上面三种对用户的影响完全不同 ——
一个要让用户立刻改变预期("这不是答案"),一个只是"排序差一点",
用同一个标记糊过去,前端没法区分该怎么展示。
`code` 给程序做分支判断,`message` 直接展示给人看。

### 引用列表的两个细节

- **返回顺序是编号升序**(1、2、3…),不是"答案里提到的顺序"。
  答案正文里的 `[1][2]` 是**纯文本**,和下面的引用列表靠编号对应 ——
  目前没有做"点正文编号跳到对应引用"的联动,那是可以改进的地方。
- **列表里那个分数的含义,取决于精排是否可用**:
  ...
  两者量纲完全不同。所以前端会跟着降级状态切换标签文案
  ("相关度" / "融合分"),免得用户把 0.032 读成"这条几乎不相关"。
...
| **降级演练** | 靠改配置 / 停容器手动触发。**验证时要用一个全新的问题** —— 问旧问题会命中缓存,你验证到的是缓存逻辑而不是降级逻辑(这个坑踩过两次) | 专门的故障注入开关,**定期演练降级路径** —— 没演练过的降级等于没有降级 |
| **可观测性** | 日志 | 接入 Prometheus 指标(检索延迟、缓存命中率、各依赖错误率、**按 code 分类的降级率**)+ 链路追踪 |
